#+TAGS: Analysis(a) DataScience(D) NLP(N) Software_Development(S) Society(s) Pandas(þ)
#+TAGS: Philosophical(P) PyTorch(p) fastai (f) tensorflow(t) Statistical(ſ) Augmentations(æ)
#+TAGS: Jupyter_Notebook(J) Kaggle(K) Collaboration(C)
